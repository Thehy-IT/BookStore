...
<div class="container" style="padding-top: 100px;">
    <div class="p-5 rounded-4 text-white position-relative overflow-hidden shadow-lg"
        ...
        <i class="fas fa-book-reader position-absolute bottom-0 end-0 mb-n4 me-n4 opacity-10"
        ...
        <div class="col-lg-3">
            <div class="sidebar-glass">
                <div class="filter-title">
                    ...
                    <a href="Product.php" class="text-decoration-none text-muted small" style="font-size: 0.8rem;">Xóa
                        lọc</a>
                </div>
                ...
                <form action="Product.php" method="GET" class="mb-4" id="searchFormSidebar">
                    <div class="input-group">
                        <input type="text" name="keyword" class="form-control border-end-0 bg-light"
                            placeholder="Tìm trong trang..."
                            value="<?php echo isset($_GET['keyword']) ? htmlspecialchars($_GET['keyword']) : ''; ?>">
                        <button class="btn btn-light border border-start-0" type="submit"><i
                ...
                <form id="filterForm" action="Product.php" method="GET">
                    ...
                    <?php if (!empty($current_category))
                        echo '<input type="hidden" name="category" value="' . htmlspecialchars($current_category) . '">'; ?>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="rating" value="4" id="star4"
                                onchange="this.form.submit()" <?php if (isset($_GET['rating']) && $_GET['rating'] == 4)
                                    echo 'checked'; ?>>
                            <label class="form-check-label text-warning" for="star4"><i class="fas fa-star"></i><i
                                    class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i
                                    class="far fa-star"></i> &amp; trở lên</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="rating" value="3" id="star3"
                                onchange="this.form.submit()" <?php if (isset($_GET['rating']) && $_GET['rating'] == 3)
                                    echo 'checked'; ?>>
                            <label class="form-check-label text-warning" for="star3"><i class="fas fa-star"></i><i
            ...
            <div class="d-flex justify-content-between align-items-center mb-4 p-3 rounded-3"
                ...
                <div class="d-flex gap-2 align-items-center">
                    <form id="sortForm" action="Product.php" method="GET" class="d-flex align-items-center">
                        ...
                        <select name="sort" class="form-select form-select-sm border-0 bg-white shadow-sm"
                            style="width: 150px;" onchange="this.form.submit()">
                            <option value="newest" <?php if ($sort_option == 'newest')
                                echo 'selected'; ?>>Mới nhất
                            </option>
                            <option value="price_asc" <?php if ($sort_option == 'price_asc')
                                echo 'selected'; ?>>Giá tăng
                                dần</option>
                            <option value="price_desc" <?php if ($sort_option == 'price_desc')
                                echo 'selected'; ?>>Giá
                                giảm dần</option>
                            <option value="bestseller" <?php if ($sort_option == 'bestseller')
                                echo 'selected'; ?>>Bán
                                chạy</option>
                        </select>
                    </form>
                    <div class="btn-group shadow-sm ms-2">
                        <button class="btn btn-white btn-sm active" id="btnGridView"><i
                            ...
                            <div class="product-card">
                                ...
                                <div class="card-img-top-wrapper">
                                    <img src="<?php echo $img; ?>"
                                        onerror="this.src='https://placehold.co/400x600/eee/31343C?text=No+Image'"
                                        alt="<?php echo $row['Title']; ?>">
                                    <div class="card-actions">
                                        <button class="btn-action" onclick='openQuickView(<?php echo json_encode($row); ?>)'
                                            title="Xem nhanh" data-bs-toggle="tooltip"><i class="fas fa-eye"></i></button>
                                        <button onclick="addToWishlist('<?php echo $row['PID']; ?>')" class="btn-action"
                                            title="Yêu thích" data-bs-toggle="tooltip"><i class="fas fa-heart"></i></button>
                                        <button onclick="addToCartAjax('<?php echo $row['PID']; ?>', 1)" class="btn-action"
                                            title="Thêm vào giỏ" data-bs-toggle="tooltip"><i
                                ...
                                <div class="card-body-glass">
                                    <div class="book-category">
                                        <?php echo $category_translations[strtolower($row['Category'])] ?? ucfirst($row['Category']); ?>
                                    </div>
                                    <h5 class="book-title text-truncate" title="<?php echo htmlspecialchars($row['Title']); ?>">
                                        <a href="description.php?ID=<?php echo $row['PID']; ?>"
                                            class="text-decoration-none text-dark"><?php echo $row['Title']; ?></a>
                                    </h5>
                                    ...
                                    <div class="price-wrapper">
                                        ...
                                        <div class="text-warning small">
                                            ...
                                            <?php echo number_format((float) rand(40, 50) / 10, 1); ?>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <ul class="pagination">
                        <?php if ($page > 1) { ?>
                            <li class="page-item"><a class="page-link"
                                    href="Product.php?<?php echo $pagination_query_string . ($page - 1); ?>"><i
            ...
            <div class="modal-body p-0">
                <div class="row g-0">
                    ...
                    <div class="col-md-7 p-4">
                        ...
                        <p class="small text-muted mb-4" id="qv-desc">Mô tả ngắn của sách sẽ được hiển thị ở đây để
                            người dùng có cái nhìn tổng quan nhanh chóng.</p>
